...
const Sidebar = () => {
  return (
    <aside
      className={
        "bg-gradient-to-b w-72 border-x-2 border-x-[#858282] from-[#4b5265] top-0 left-0 h-full"
      }
    >
      <div
        className={
      ...
      <div className={"pt-12 w-full flex flex-col gap-8"}>
        <SidebarButton title={"Магазин Паймон"} param={"paimonBargain"}>
          <PaimonBargainIcon param={"paimonBargain"} />
        </SidebarButton>
        <SidebarButton
          title={"Пополнение кристаллов"}
          param={"genesisCrystals"}
        >
          <GenesisIcon param={"genesisCrystals"} />
        </SidebarButton>
      </div>
    </aside>
  );
