...
const HistoryTable = () => {
  ...
  if (history.length === 0) {
    return (
      <p
        className={"absolute text-[#595252] text-[2.1vw] top-[45%] left-[20%]"}
      >
  ...
  return (
    <>
      <div
        className={
          "absolute w-[81.5%] h-[60%] top-[24%] left-[9%] overflow-y-scroll scrollbar-for-epitomized-path"
        }
      >
        <p className={"text-[1.7vw] md:text-[1vw] text-[#9a8e8e]"}>
          Всего молитв сделано: {history.length}
        </p>
        <div className={"flex gap-3.5 md:gap-12"}>
          <div>
            <p className={"text-[2.2vw] md:text-[1vw] text-[#9659c7]"}>
              Всего предметов 4★ получено:{" "}
              {history.filter((wish) => wish.item.rare === 4).length}
            </p>
            <p className={"text-[2.2vw] md:text-[1vw] text-[#bd6932]"}>
        ...
        <table
          className={
            "absolute border mt-2 mr-2 border-[#dac69f] text-[3vw] md:text-[1.2vw] leading-6"
          }
        >
          <thead>
