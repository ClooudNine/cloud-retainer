import { EpitomizedPath } from '@/lib/common';
import { BannerItems, Banners, bannerTranslates, WishHistory } from '@/lib/banners';
            ...
            if (epitomizedPath[banner.id]) {
                if (epitomizedPath[banner.id].count === 2)
                    returnedWeapon = items.filter(
                        (item) =>
                            item.id === epitomizedPath[banner.id].weaponId &&
                            item.rare === '5'
                    ) as Weapon[];
