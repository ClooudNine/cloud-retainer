...
export default function GenesisCrystals() {
    ...
    return (
        <>
            <header className={'flex items-center justify-end gap-8 px-6 py-6 text-2xl xs:py-4 lg:text-base'}>
                <Balance
                ...
                {crystalsCount.map((count) => (
                    ...
                    >
                        <Image
                            ...
                            alt={t('image-alts.genesis-crystals-shop-background')}
                            fill
                            priority
                            draggable={false}
                        />
                        ...
                        />
                        <p className={'absolute top-[55%] text-[#585f6b] text-center px-2'}>
                            {t('common.genesis-crystal.title', { count: count })}
                        </p>
                        <p className={'absolute top-[84%] text-[#585f6b]'}>{t('wish-simulator.free')}</p>
                    </div>
                ))}
                {obtainedCrystals && (
                    <ObtainModal
