...
const Navigation = ({ bannerType }: { bannerType: BannerTypes }) => {
    ...
    const sections: {
        ...
        'increased-chance': t('increased-chance'),
    ...
    return (
        ...
        >
            <div className={'w-full h-full flex justify-between items-center border-y border-y-[#ede5d7]'}>
                {Object.entries(sections).map((section) => (
                    <NavigationButton
                        key={section[0]}
                        title={section[1]}
                        param={section[0] as 'increased-chance' | 'more-info' | 'items-list'}
                    />
                ))}
            </div>
        </nav>
