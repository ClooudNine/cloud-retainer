...
const ChapterList = () => {
    ...
    const filteredChapters = useMemo(() => {
        return achievements.filter((chapter) =>
            t(`achievements-chapter.${chapter.title}`).toLowerCase().includes(searchChapter.toLowerCase())
        );
    }, [achievements, searchChapter, t]);
    ...
    return (
        ...
        >
            <p>
                {t('main.total-chapters')}: {achievements.length}
            </p>
            <div className={'flex items-center gap-2'}>
                <Search className={'h-full w-auto'} />
                <Input
                    type={'text'}
                    placeholder={t('main.search-chapters')}
